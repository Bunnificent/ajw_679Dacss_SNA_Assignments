...
igEL_Den <- graph.density(igEL)
 ...
 graph.density(igEL_randoGraph)
 edge_density(igEL_randoGraph)
...
igEL_triList <- triad_census(igEL)
igEL_triList

sum(
  triad_census(igEL)
)
summary(igEL_triList)
...
network::has.loops(snEL)
 ...
 # computes the density of  graphs and adjusts for the type of graph 
  ...
  gden(
    snEL, 
       g=NULL, 
       diag=T, # Set to TRUE if graph contains loops
...
-   identify particular nodes (such as the five
    nodes with the most contacts). 
    **Vertex degree** is a description of an individual
...
A dyad in an undirected network can have two unique configurations: connected or disconnected; 
...
```{r Statnet:Centrality: High Degree Nodes}
library(statnet)
library(sna)
#sort to find 5 highest and lowest degree nodes in trade100

snel_HiStatus<-data.frame(
  name=snEL%v%"vertex.names",
  sna::degree(snEL),
  indegree=sna::degree(snEL, cmode="indegree"),
  outdegree=sna::degree(snEL, cmode="outdegree"))
...
```{r Statnet: Centrality: Low Degree Nodes}
library(statnet)
library(sna)
#sort to find 5 highest and lowest degree nodes in trade100
