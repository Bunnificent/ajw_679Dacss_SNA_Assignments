...
#   Project Varriables
  ...
  indx_SmugglerCove_recip
    ...
    DataFrame:
  smugCovIdx
            colname: "dnkName" "dnkIng"  "ingType"
...
```{r EDGELIST: UNDIRECTED Network Objects: statnet & igraph, include=FALSE}
...
```{r EDGELIST: IGRAPH: DIRECTED Network: ingType, include=FALSE}
...
network_edgelist <- smug_matrix
  ...
  nodes_ingType <- smugCovIdx 
    nodes_ingType<- 
      select(nodes_ingType,ingType)
view(nodes_ingType)
        ### View(nodes_ingType)
...
vertex.attributes(d_network_ig)
                ## create new edge attribute
  E(d_network_ig)$dnkIng<-smugCovIdx$dnkIng
  E(d_network_ig)$dnkName<-smugCovIdx$dnkName
 
  #create vertex attributes in igraph
 V(d_network_ig)[match(V(d_network_ig)$ingType,nodes_ingType$ingType)]$ingType<- nodes_ingType$ingType
  
   View(d_network_ig)
   head(d_network_ig)
  summary(d_network_ig)
  summary(edge_attr(d_network_ig))
  summary(vertex.attributes(d_network_ig))
    
   ingTypeEL_Dig <- d_network_ig
```
...
```{r IGRAPH: DIRECTED Network: ingType, echo=TRUE}
...
## Network Size
      ...
      # Count Vertices
    vcount(ingTypeEL_Dig)
    
      # Count Edges
    ecount(ingTypeEL_Dig)

## Network Mode
    # Bipartitie / Unimodal
    is_bipartite(ingTypeEL_Dig)

## Edges    
    # Un/directed 
    is_directed(ingTypeEL_Dig)

    # weighted or unweighted?
    is_weighted(ingTypeEL_Dig)      
    
    # edge weights
    E(ingTypeEL_Dig)$weight

## access vertex attributes
    
    # Generate a list the names of vertex attributes
  V(ingTypeEL_Dig)$name

    # access edge attribute  names
  igraph::vertex_attr_names(ingTypeEL_Dig)

    # Generate a list the names of edge attributes  
  igraph::edge_attr_names(ingTypeEL_Dig)
  

## Network Attributes
 summary(ingTypeEL_Dig)
          # Vertices =  
          # Directed = 
          # hyper = 
          # loops = 
...
#     find average shortest path for network
    average.path.length(ingTypeEL_Dig,directed=T)    
...
#     isolating the distances between specific node
    all_shortest_paths(ingTypeEL_Dig,
                       "Strozzi","Tornabuoni", 
                       weights=NA)$res
## Note: manually tell igraph to ignore edge weights.
```   
...
#     Calculate distances between two nodes
    distances(ingTypeEL_Dig,"Bischeri","Ridolfi")

#     Calculate distance between two nodes using unweighted edges
    distances(ingTypeEL_Dig,"Bischeri", "Castellani",weights=NA)
...
```{r iGraph: Undirected: Component Structure, echo=TRUE}
 ...
 igraph::components(smugEL_ig)$no 
      ##**NOTE:** $no prevents a list of comonents from being printed
...
V(smugNet)$dnkIng <- 
  smug_attributes$dnkIng[match(V(smugNet)$name, smug_attributes$ingType)]

V(smugNet)$dnkName <- 
  smug_attributes$dnkName[
    match(V(
      smugNet)$name,
...
V(smugNet)$ingType <- 
  smug_attributes$ingType[match(V(smugNet)$name, smug_attributes$ingType)]

V(smugNet)$dnkName <-
  smug_attributes$dnkName[match(V(smugNet)$name, smug_attributes$dnkName)]
...
```{r iGraph: Undirected: DEGREE CENTRALITY: ingType, echo=TRUE}
# graph the SMUG network 
...
V(smugNet)$dnkName <-
  smug_attributes$dnkName[match(V(smugNet)$name, smug_attributes$dnkIng)]
...
ingTypeEL_DigNod <- vcount(ingTypeEL_Dig)

ingTypeEL_DigDen <- graph.density(ingTypeEL_Dig)
...
view(degree(ingTypeEL_Dig))

ingT_deg <- ingTypeEL_Dig
V(ingT_deg)$degree <- degree(ingT_deg) # assignment

# plot(ingT_deg, 
#     vertex.label.cex = .6, 
#     vertex.label.color = "black", 
#     vertex.size = V(ingTypeEL_Dig)$degree, 
...
view(betweenness(ingTypeEL_Dig, directed = FALSE))

# view(betweenness(ingTypeEL_Dig, directed = TRUE))
              # Yeilds 0

ingTypeEL_Between <- ingTypeEL_Dig
# We can again assign the output of betweenness() to a variable in the network and size the nodes according to it.

V(ingTypeEL_Between)$betweenness <- 
  betweenness(ingTypeEL_Between, 
              directed = F)
...
view(evcent(ingTypeEL_Dig)$vector)
# view(evcent(smugEL_ig)$vector)
ingTyp_Vector <- ingTypeEL_Dig
